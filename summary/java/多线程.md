...
然后调用`acquireQueued()`从队列中获取(**这是个核心方法，最好记住**)，当阻塞线程节点被`unparkSuccessor`唤醒时会继续循环，并调用`tryAcquire`尝试获取同步状态
...
如上图所示，每个没有获取到同步状态(阻塞)的节点线程都在等待`head`节点释放同步状态，当其释放状态时会唤醒后继节点(`unparkSuccessor`)，后继节点被唤醒后便可以继续循环尝试获取同步状态(`acquireQueued`继续执行)，如果获取成功(`tryAcquire`返回`true`)，该节点将成为`head`节点，并将原来的`head`节点回收。
...
`doAcquireShared`方法将`tryAcquireShared`失败的线程加入同步队列并挂起，同时不断的自旋(**这是个核心方法，最好记住**)。
        ...
        try {
            ...
            for (;;) {
                ...
                if (p == head) {
                    ...
                    if (r >= 0) {
                        //(1)获取成功，则将node设置为head节点并propagate，传递意思就是会继续唤醒当前节点的后继节点
...
如上图所示，假设`head`节点后面的所有节点都处于阻塞状态(`tryAcquireShared`返回负值)，当调用`releaseShared`方法释放同步状态时，如果尝试释放同步状态成功(`tryReleaseShared`返回`true`)，那么会唤醒后继节点(`doReleaseShared/unparkSuccessor`)，后继节点被唤醒后便会从阻塞处返回(继续执行`acquireShared`的代码)，尝试重新获取同步状态(`tryAcquireShared`)，如果获取成功，那么会将该节点设置为`head`节点，并继续唤醒后继节点。

对于`Semaphore`，在释放同步状态(调用`release`方法)时，实际上就是简单的增加了同步状态(剩余许可增加释放量`available = current - release`)，然后唤醒`head`节点后继节点线程，该节点线程便会继续执行，尝试继续获取同步状态，`tryAcquireShared(int arg)`有多种实现，其核心代码就是返回可用的同步状态(`remaining = available - acquires`)，如果`remaining >= 0`，则表示成功，可以唤醒后继节点，更新`head`节点(`setHeadAndPropagate`)，后面对于每个节点重复同样的操作，最理想的情况是一次唤醒就顺序唤醒了所有的阻塞节点线程(意味着后面的所有节点线程`tryAcquireShared`都成功了)，例如对于信号量`release(int arg)`传入了足够大的参数，后面所有的阻塞线程节点都能获得自己的许可数量，那么会被全部唤醒；而对于`CountDownLatch`而言则更简单，其内部的`tryReleaseShared`实现当且仅当同步状态设置为0时才会返回`true`，对照着`tryAcquireShared`方法，当`state == 0`时返回1(正值，那么会唤醒后继节点)，否则返回-1(负值，不会唤醒后继节点)，这也是`CountDownLatch`的特点，每次调用`countDown`方法都会减少一次`state`，当`state`减少到0时就会顺序唤醒所有阻塞线程。
