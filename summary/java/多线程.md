...
设置了`corePoolSize=maximumPoolSize=nThreads`，`keepAliveTime=0`，任务队列选用的是无边界的`LinkedBlockingQueue`，可以看出该线程池具有以下特性：线程池中最多会存在`nThreads`个线程，每来一个新任务，如果线程池中线程数量小于`nThreads`，那么会新建线程执行该任务，否则任务进入队列，等待空闲线程
