...
虚拟机在响应两类事件时会关闭：
 ...
 - 程序正常退出，**当最后一个非`daemon`线程退出(即使还有`daemon`线程在运行)或者调用`Runtime.getRuntime().exit()/System.exit()`**
 - 虚拟机响应用户中断而终止，例如输入`Ctrl + C`或者系统范围的事件，例如用户退出或者系统关闭

`shutdown hook`是一个**已经初始化但还没有启动的线程**，当`jvm`开启它的关闭流程时，它会以非确定的顺序并发地开始所有注册的`shutdown hook`线程的运行，当钩子线程执行完毕，如果`finalization-on-exit`为真。虚拟机会执行所有没有调用的`finalizer`，值得注意的是，所有的`daemon`线程在关闭流程过程中仍然会继续运行，非`deamon`线程也是。

如果虚拟机关闭流程开始，那么只能通过调用`Runtime.getRuntime().halt()`方法来终止流程，这将会强制终结`JVM`，在关闭流程开始后，不能再注册新的`shutdown hook`和解注册关闭钩子，这些操作会抛出`IllegalStateException`异常。

关闭钩子线程运行在`JVM`生命周期非常脆弱的时期，因此需要**保证其线程安全且要尽量避免死锁**，不能依靠那些已经注册了自己的关闭钩子的服务，这些服务往往会处在关闭的过程中，并且尝试使用其他线程依赖的服务例如`AWT`事件派发线程会导致死锁。
