...
指发生在老年代的垃圾收集动作，出现了`Major GC`，经常会伴随至少一次`Minor GC`(但不是绝对的)，`Major GC`的速度一般比`Minor GC`慢上10倍以上
...
需要计算是否需要显示调用`System.gc()`方法，如果没必要就移除该调用代码，另外，可以通过`-XX:+DisableExplicitGC`虚拟机参数禁止显示屌用`System.gc()`
